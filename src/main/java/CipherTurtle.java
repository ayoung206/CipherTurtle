 ...
 *
 * This class is to provide a running animation by writing each of the
 * encrypted character of the String out. This class will use single thread
 * so that one turtle will draw all characters.
 */
...
/**************************************/
  ...
  /**
   * Name: encrypt(String originalStr, int rotation)
   * This method checks if a String is containing only capital letters, encrypt it,
   * and tell the Turtle to draw it.
   * @param String originalStr: letter being encrypted
   * @param int rotation: amount of rotation to encrypt
   * @return void
   */
  private static void encrypt(String originalStr, int rotation) {
    ...
    //loop through the every character of originalStr
    for (int i = 0; i < originalStr.length(); i++) {
        char c = originalStr.charAt(i);

        //if the String is not capital or is empty
        if (!Character.isUpperCase(c) || originalStr == null ||originalStr.isEmpty()) {
            System.err.println(errorMessage);
            return;
        }
        encrypted.append(letterOperation(c, rotation));
    }

    int x = WORLD_WIDTH/CHAR_SPACE;
    int y = WORLD_WIDTH/CHAR_SPACE;
    // Draw every letter
    for (int i = 0; i < encrypted.toString().length(); i++) {
